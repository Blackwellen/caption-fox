...
import {
  canAccessCommunityModule, communityCapabilities, visibleCommunityModules,
  type CommunityCapabilities, type CommunityContext,
} from './entitlements'
import type { CommunityModule } from './constants'
 ...
 * Community module. Redirects unauthenticated users to login and users with
 * no workspace to onboarding — every Community route funnels through this, so
 * a pasted deep link is protected rather than merely hidden from the nav.
 */
export async function getCommunitySession(): Promise<CommunitySession> {
  ...
  return {
    ...
    capabilities: communityCapabilities(ctx),
    modules: visibleCommunityModules(ctx),
  }
}

/**
 * Same as `getCommunitySession`, plus a hard route-level gate for one module.
 * Returns the module's block reason instead of throwing so the page can
 * render the canonical no-access / upgrade state rather than a 404.
 */
export async function requireCommunityModule(module: CommunityModule) {
  const session = await getCommunitySession()
  const access = canAccessCommunityModule(session.ctx, module)
