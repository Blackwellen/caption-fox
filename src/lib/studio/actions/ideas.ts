...
export interface IdeaInput {
  ...
  nextStep?: string
  whyItWorks?: string[]
  /** Where the idea came from, e.g. "Google Trends". */
  sourceLabel?: string
}
...
export async function saveIdea(input: IdeaInput): Promise<ActionResult> {
  ...
  if (input.id) {
    ...
    // Stage changes go through setIdeaStage so the workflow rules always apply.
    const { stage: _ignored, ...editable } = payload
    void _ignored
    const { data: current } = await supabase.from('content_ideas').select('metadata').eq('id', input.id).eq('workspace_id', ctx.workspaceId).single()
    const metadata = { ...((current?.metadata ?? {}) as Record<string, unknown>), ...(input.sourceLabel !== undefined ? { source_label: input.sourceLabel.trim().slice(0, 80) || null } : {}) }
    const { error: updateError } = await supabase
      .from('content_ideas').update({ ...editable, metadata }).eq('id', input.id).eq('workspace_id', ctx.workspaceId)
  ...
  const { data, error: insertError } = await supabase
    .from('content_ideas')
    .insert({ ...payload, owner_id: userId, created_by: userId, status: 'idea', metadata: { source_label: input.sourceLabel?.trim().slice(0, 80) || null } })
    .select('id').single()
  ...
  await logActivity(supabase, ctx.workspaceId, userId, {
    ...
    summary: `added the idea “${title}”`, link: `${studioBase(ctx)}/ideas?selected=${data.id}`,
  })
  revalidateStudio()
...
export async function setIdeaStage(input: { id: string; stage: string }): Promise<ActionResult> {
  ...
  await logActivity(supabase, ctx.workspaceId, userId, {
    ...
    link: `${studioBase(ctx)}/ideas?selected=${input.id}`,
  })
...
/** Adds a follow-up task to an idea. The idea must belong to this workspace. */
export async function addIdeaTask(input: { ideaId: string; title: string; dueOn?: string | null }): Promise<ActionResult> {
  const { session, error } = await authorise('editIdeas')
  if (!session) return fail(error!)
  const { supabase, ctx, userId } = session
  const title = input.title?.trim() ?? ''
  if (!title) return fail('Describe the task.', { title: 'A task needs a title.' })
  if (title.length > 200) return fail('Tasks are limited to 200 characters.', { title: 'Too long.' })
  if (input.dueOn && !/^\d{4}-\d{2}-\d{2}$/.test(input.dueOn)) return fail('Enter a valid due date.', { dueOn: 'Invalid date.' })
  if (!(await ownsRecord(supabase, 'content_ideas', input.ideaId, ctx.workspaceId))) return fail('That idea is not part of this workspace.')

  const { data, error: insertError } = await supabase.from('studio_idea_tasks').insert({
    workspace_id: ctx.workspaceId, idea_id: input.ideaId, title, due_on: input.dueOn || null, created_by: userId, assignee_id: userId,
  }).select('id').single()
  if (insertError) return dbFail('addIdeaTask', insertError)
  await logAudit(supabase, ctx.workspaceId, userId, { action: 'studio.ideas.task_added', resourceType: 'studio_idea_task', resourceId: data.id, metadata: { idea_id: input.ideaId } })
  revalidateStudio()
  return { ok: true, id: data.id, message: 'Task added.' }
}

/** Marks an idea task done (or reopens it). */
export async function setIdeaTaskDone(input: { id: string; done: boolean }): Promise<ActionResult> {
  const { session, error } = await authorise('editIdeas')
  if (!session) return fail(error!)
  const { supabase, ctx, userId } = session
  const { data: task } = await supabase.from('studio_idea_tasks').select('id, title, idea_id')
    .eq('id', input.id).eq('workspace_id', ctx.workspaceId).maybeSingle()
  if (!task) return fail('That task is not part of this workspace.')
  const { error: updateError } = await supabase.from('studio_idea_tasks')
    .update({ done_at: input.done ? new Date().toISOString() : null, done_by: input.done ? userId : null })
    .eq('id', input.id).eq('workspace_id', ctx.workspaceId)
  if (updateError) return dbFail('setIdeaTaskDone', updateError)
  if (input.done) {
    await logActivity(supabase, ctx.workspaceId, userId, {
      entityType: 'idea', entityId: task.idea_id, action: 'task_done', summary: `completed “${task.title}”`,
      link: `${studioBase(ctx)}/ideas?selected=${task.idea_id}`,
    })
  }
  revalidateStudio()
  return { ok: true, message: input.done ? 'Task completed.' : 'Task reopened.' }
}
