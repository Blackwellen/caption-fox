...
export async function saveKeywordSet(input: KeywordSetInput): Promise<ActionResult> {
  ...
  await logActivity(supabase, ctx.workspaceId, userId, {
    ...
    link: `${studioBase(ctx)}/hashtags?selected=${data.id}`,
  })
  revalidateStudio()
...
/** Permission check and audit record for a client-side CSV export of keyword groups. */
export async function logKeywordExport(input: { count: number }): Promise<ActionResult> {
  const { session, error } = await authorise('exportHashtags')
  if (!session) return fail(error!)
  const { supabase, ctx, userId } = session
  await logAudit(supabase, ctx.workspaceId, userId, {
    action: 'studio.hashtags.exported', resourceType: 'hashtag_set',
    metadata: { count: Math.max(0, Math.min(10_000, Math.round(Number(input.count) || 0))) },
  })
  return { ok: true }
}

/** Archives (or restores) a cluster or hashtag set. Terms are kept for restore. */
export async function archiveKeywordSet(input: { id: string; restore?: boolean }): Promise<ActionResult> {
  const { session, error } = await authorise('editHashtags')
  if (!session) return fail(error!)
  const { supabase, ctx, userId } = session

  const { data: current } = await supabase.from('hashtag_sets').select('id, name')
    .eq('id', input.id).eq('workspace_id', ctx.workspaceId).maybeSingle()
  if (!current) return fail('That group is not part of this workspace.')

  const { error: updateError } = await supabase.from('hashtag_sets')
    .update({ archived_at: input.restore ? null : new Date().toISOString() })
    .eq('id', input.id).eq('workspace_id', ctx.workspaceId)
  if (updateError) return dbFail('archiveKeywordSet', updateError)

  await logAudit(supabase, ctx.workspaceId, userId, {
    action: input.restore ? 'studio.hashtags.restored' : 'studio.hashtags.archived',
    resourceType: 'hashtag_set', resourceId: input.id, metadata: { name: current.name },
  })
  revalidateStudio()
  return { ok: true, message: input.restore ? `“${current.name}” restored.` : `“${current.name}” archived.` }
}
