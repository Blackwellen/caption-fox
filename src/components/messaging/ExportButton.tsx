 ...
 * and channel are forwarded to the server, which re-applies permissions
 * before writing a row. Disabled with a reason when the role cannot export.
 */
export default function ExportButton({
  entity, allowed, className,
}: { entity: 'messages' | 'templates' | 'journeys' | 'audiences'; allowed: boolean; className?: string }) {
  ...
  return (
    <a
      href={`/app/messaging/export?${search.toString()}`}
  ...
  return (
    <div className="relative">
      <button
        ...
        aria-haspopup="menu" aria-expanded={open} aria-label="More messaging actions"
