...
import {
  Archive, CheckCircle2, FileEdit, Mail, MessageSquare, Plus, Trash2, Upload, Workflow,
} from 'lucide-react'
import { cn, formatRelative } from '@/lib/utils'
import type { ActivityRow } from '@/lib/messaging/types'

/** Icon + tone per activity entity, so the feed scans quickly. */
function iconFor(item: ActivityRow): { Icon: typeof Plus; tone: string } {
  if (item.entity_type === 'journey') return { Icon: Workflow, tone: 'bg-violet-50 text-violet-600' }
  if (item.entity_type === 'template') return { Icon: FileEdit, tone: 'bg-blue-50 text-blue-600' }
  if (item.entity_type === 'audience' || item.entity_type === 'contact') return { Icon: Upload, tone: 'bg-sky-50 text-sky-600' }
  ...
  if (item.action.includes('approve') || item.action.includes('sent')) return { Icon: CheckCircle2, tone: 'bg-emerald-50 text-emerald-600' }
  if (item.entity_type === 'channel') return { Icon: MessageSquare, tone: 'bg-amber-50 text-amber-600' }
  return { Icon: Mail, tone: 'bg-slate-100 text-slate-500' }
}

export default function ActivityFeed({
  items, emptyMessage = 'No messaging activity yet. Actions you take here will appear in this feed.',
  className,
