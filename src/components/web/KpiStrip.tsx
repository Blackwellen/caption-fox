import {
  AlertTriangle, ArrowDownRight, ArrowUpRight, CheckCircle2, FlaskConical, Gauge, Globe2,
...
const ICONS = {
  pages: Globe2, forms: CheckCircle2, funnel: TrendingUp, experiment: FlaskConical,
  leads: Users, health: ShieldCheck, alert: AlertTriangle, gauge: Gauge,
} as const
...
/**
 * The KPI row shared by every Web & Conversion surface. Values are always
...
export default function KpiStrip({ items, className }: { items: KpiValue[]; className?: string }) {
  return (
    <div className={cn(CARD, CARD_SHADOW, 'grid grid-cols-2 divide-slate-200 sm:grid-cols-3 sm:divide-x xl:grid-cols-6', className)}>
      {items.map(item => {
        const Icon = ICONS[(item.icon as KpiIcon)] ?? Globe2
        return (
