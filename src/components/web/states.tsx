...
import { AlertTriangle, Globe2, Lock, SearchX, Sparkles } from 'lucide-react'
import { cn } from '@/lib/utils'
import { CARD, CARD_SHADOW } from './primitives'
import type { ModuleAccess } from '@/lib/web/entitlements'

/**
 * Canonical blocked / upgrade state for a Web & Conversion module the
 * workspace or role cannot open. Rendered by the route itself, so a direct
 * URL is protected rather than merely hidden from navigation.
 */
...
export function WebEmpty({
  title, message, action, icon = 'globe', bare = false, className,
}: {
  title: string
  message: string
  ...
  const Icon = icon === 'search' ? SearchX : Globe2
  return (
    ...
    <div className={cn(CARD, 'border-red-200 bg-red-50/50 px-4 py-3', className)}>
      <p className="flex items-start gap-2 text-[13px] text-red-800">
        ...
        <span>
          ...
          <code className="rounded bg-red-100 px-1 font-mono text-[11px]">CF-WEB</code>.
        </span>
      </p>
    </div>
  )
