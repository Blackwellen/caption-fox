...
import { formatRelativeShort, formatShortDate, timezoneAbbrev } from '@/lib/calendar/dates'
  ...
  return (
    <div className={cn(T.page, 'py-6')}>
      <CalendarPageChrome
        ...
        actions={
          <SecondaryHeaderActions
            ctx={ctx}
            surface="conflicts"
            primary={<ConflictPrimaryActions ctx={ctx} conflicts={list.data.conflicts} lookups={lookups.data} />}
          />
        }
      />
      ...
      <FilterBar
        left={
          <>
            <FilterSearch placeholder="Search conflicts…" className="w-56" />
            <FilterSelect name="severity" label="Severity" options={SEVERITY_OPTIONS} />
            ...
            <FilterSelect name="type" label="Conflict type" options={TYPE_FILTER_OPTIONS} />
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-3">
        ...
        <Panel title="Recent resolution activity" action={{ label: 'View all', href: `${ctx.basePath}/calendar/conflicts?status=resolved` }} bodyClassName="divide-y divide-slate-100">
          {activity.error ? <ErrorState message={activity.error} /> : activity.data.length === 0 ? (
            ...
            <div key={item.id} className="flex items-start gap-2.5 px-5 py-2.5 first:pt-4 last:pb-4">
              ...
              <div className="min-w-0 flex-1">
                <Link href={item.href ?? '#'} className={cn('block truncate text-[12.5px] font-medium text-slate-800 hover:text-blue-600', T.focus)}>
                  {item.summary}
                </Link>
      ...
      <p className="mt-5 text-[11.5px] text-slate-400">
        All times shown in {ctx.timezone} ({timezoneAbbrev(ctx.timezone)}). Recommended actions are generated from the detection rules for each conflict type and should be reviewed before applying.
      </p>
    </div>
  )
}
        ...
        <tbody className="divide-y divide-slate-100">
          {conflicts.map(conflict => (
            <tr key={conflict.id} className="hover:bg-slate-50">
              <td className="whitespace-nowrap px-5 py-2.5 text-[12px] font-medium text-slate-500">{conflict.reference}</td>
              <td className="max-w-[220px] px-3 py-2.5">
                <Link href={`${ctx.basePath}/calendar/conflicts?selected=${conflict.id}`} className={cn('block truncate text-[12.5px] font-medium text-slate-900 hover:text-blue-600', T.focus)}>
                  {conflict.title}
                </Link>
              </td>
              <td className="whitespace-nowrap px-3 py-2.5 text-[12px] text-slate-600">{CONFLICT_TYPE_LABELS[conflict.type]}</td>
              <td className="px-3 py-2.5"><SeverityBadge severity={conflict.severity} /></td>
              <td className="px-3 py-2.5">
                <span className="flex gap-1">
                  {conflict.channels.length === 0 ? <span className="text-[12px] text-slate-400">—</span>
                    : conflict.channels.slice(0, 3).map(channel => (
                      <span key={channel} className="flex items-center gap-1 text-[12px] text-slate-600">
              ...
              <td className="max-w-[160px] truncate px-3 py-2.5 text-[12px] text-slate-600">{conflict.campaignName ?? '—'}</td>
              <td className="px-3 py-2.5">
                <span className="flex items-center gap-1.5">
                  <Avatar name={conflict.ownerName} size={20} />
                  <span className="hidden max-w-[100px] truncate text-[12px] text-slate-700 2xl:block">{conflict.ownerName ?? 'Unassigned'}</span>
                </span>
              </td>
              ...
              <td className="px-3 py-2.5">
                <span className={cn('rounded-full px-2 py-0.5 text-[11px] font-medium', STATUS_CHIP[conflict.status])}>
                  {STATUS_LABEL[conflict.status]}
                </span>
              </td>
              <td className={cn('px-5 py-2.5 text-[12px] font-semibold capitalize',
    ...
    <div>
          ...
          <thead>
            <tr>
              ...
              {data.buckets.map(bucket => (
                <th key={bucket} scope="col" className="pb-1 text-center text-[10px] font-medium text-slate-400">{bucket}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {data.channels.map(channel => (
              <tr key={channel}>
                <th scope="row" className="pr-2 text-right text-[11px] font-medium capitalize text-slate-600">
      ...
      <ul className="mt-3 flex items-center justify-center gap-4 border-t border-slate-100 pt-2.5">
        {(['low', 'medium', 'high'] as const).map(level => (
          <li key={level} className="flex items-center gap-1.5 text-[11px] capitalize text-slate-500">
