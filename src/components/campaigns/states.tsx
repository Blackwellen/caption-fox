...
import { AlertTriangle, Lock, Megaphone, SearchX, Sparkles } from 'lucide-react'
import { cn } from '@/lib/utils'
import { CARD, CARD_SHADOW } from './primitives'
import type { ModuleAccess } from '@/lib/campaigns/entitlements'

/**
 * Canonical blocked / upgrade state for a Campaigns module the workspace or
 * role cannot open. Rendered by the route itself, so a direct URL is protected
...
export function CampaignsEmpty({
  title, message, action, icon = 'campaign', bare = false, className,
}: {
  title: string
  message: string
  action?: React.ReactNode
  icon?: 'campaign' | 'search'
  /** Drops the card chrome when the empty state already sits inside a Panel. */
  bare?: boolean
  className?: string
}) {
  const Icon = icon === 'search' ? SearchX : Megaphone
  return (
...
/** Surfaces a query failure without swallowing it or hanging on a spinner. */
export function LoadError({ message, className }: { message: string; className?: string }) {
  return (
    <div className={cn(CARD, 'border-red-200 bg-red-50/50 px-4 py-3', className)}>
      <p className="flex items-start gap-2 text-[13px] text-red-800">
        ...
        <span>
          ...
          <code className="rounded bg-red-100 px-1 font-mono text-[11px]">CF-CAMPAIGNS</code>.
        </span>
      </p>
    </div>
  )
