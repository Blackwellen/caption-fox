...
export function CampaignThumb({
  ...
  return (
    <span
      aria-hidden
      className={cn(
        'flex shrink-0 items-center justify-center rounded-lg bg-gradient-to-br text-[13px] font-bold text-white/90',
...
export default function CampaignCard({
  ...
  return (
    <article className={cn(CARD, CARD_SHADOW, 'group relative flex flex-col p-3 transition-shadow hover:shadow-md')}>
      {featured && (
        <span
      ...
      <div className="flex gap-2.5">
        <CampaignThumb campaign={campaign} className="h-[52px] w-[60px]" />
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-1">
            <Link
              href={`/app/campaigns/${campaign.id}`}
              className="line-clamp-1 flex-1 text-[13px] font-semibold text-slate-900 hover:text-blue-600"
            >
              {campaign.name}
            </Link>
            <CampaignActionsMenu
          ...
          <div className="mt-1.5">
            <Badge variant={LIFECYCLE_BADGE[stage] ?? 'slate'} className="text-[10px]">
      ...
      <div className="mt-2.5 flex items-center gap-2">
        <span className="w-9 shrink-0 text-[11px] font-semibold text-slate-700" title="Delivery progress">
          {campaign.progress}%
        ...
        />
        <span
          className="w-9 shrink-0 text-right text-[11px] font-medium text-slate-400"
      ...
      <dl className="mt-2.5 grid grid-cols-2 gap-2 border-t border-slate-100 pt-2.5">
        <div className="min-w-0">
          <dd className="truncate text-[13px] font-semibold text-slate-900">
            {budget > 0 ? formatMoney(budget, campaign.currency ?? 'GBP') : '—'}
          </dd>
          <dt className="text-[10px] text-slate-400">Budget</dt>
        </div>
        <div className="min-w-0 text-right">
          <dd className="truncate text-[13px] font-semibold text-slate-900">
            {formatShortDate(campaign.end_date)}
          </dd>
          <dt className="text-[10px] text-slate-400">Due date</dt>
        </div>
      </dl>
      ...
      <div className="mt-2.5 flex items-center gap-2 border-t border-slate-100 pt-2.5">
        ...
        {contentCount !== undefined && (
          <span className="ml-auto inline-flex items-center gap-1 text-[11px] text-slate-400" title="Linked content items">
            <FileText size={11} />
            {contentCount}
