...
export default function MilestoneList({
  ...
  if (milestones.length === 0) {
    return <p className="py-6 text-center text-[13px] text-slate-400">{emptyMessage}</p>
  }
        ...
        return (
          <li key={milestone.id} className="flex items-start gap-2">
            ...
            <span className="min-w-0 flex-1">
              <span className={cn('block truncate text-[12px] font-medium', done ? 'text-slate-400 line-through' : 'text-slate-900')}>
                {milestone.title}
              </span>
              <span className="block truncate text-[11px] text-slate-400">
                {milestone.campaign?.name ? (
                  <Link href={`/app/campaigns/${milestone.campaign.id}`} className="hover:text-blue-600">{milestone.campaign.name}</Link>
                ) : 'Unlinked'}
              </span>
            </span>
            <span className="shrink-0 text-[11px] text-slate-500">{formatDate(milestone.due_date)}</span>
          </li>
        )
      })}
      {pending && <li className="flex items-center gap-1.5 text-[12px] text-slate-400"><Loader2 size={12} className="animate-spin" /> Saving…</li>}
    </ul>
  )
}
