  ...
  return (
    <article className={cn(CARD, CARD_SHADOW, 'relative flex flex-col p-3 transition-shadow hover:shadow-md')}>
      {(featured || template.is_favourite) && (
        <span
          className="absolute -left-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-blue-600 text-[10px] lg:text-[8px] text-white ring-2 ring-white"
      ...
      <div className="flex gap-2.5">
        {template.cover_url ? (
          ...
          <span
            aria-hidden
            className={cn('flex h-[52px] w-[60px] shrink-0 items-center justify-center rounded-lg bg-gradient-to-br text-[13px] lg:text-[10px] font-bold text-slate-500', tintFor(template.id))}
          >
            {template.name.slice(0, 2).toUpperCase()}
          </span>
        )}
        <div className="min-w-0 flex-1">
          <h3 className="line-clamp-1 text-[13px] lg:text-[10px] font-semibold text-slate-900">{template.name}</h3>
          <p className="mt-px truncate text-[11px] lg:text-[8.5px] capitalize text-slate-400">
          ...
          <div className="mt-1.5">
            <Badge variant={TEMPLATE_STATUS_BADGE[status] ?? 'slate'} className="text-[10px] lg:text-[8px]">
      ...
      <dl className="mt-2.5 flex gap-5 border-t border-slate-100 pt-2.5">
        <div>
          <dt className="text-[10px] lg:text-[8px] text-slate-400">Used</dt>
          <dd className="text-[12px] lg:text-[9.5px] font-semibold text-slate-900">
            {template.usage_count} time{template.usage_count === 1 ? '' : 's'}
          </dd>
        </div>
        <div>
          <dt className="text-[10px] lg:text-[8px] text-slate-400">Workflows</dt>
          <dd className="text-[12px] lg:text-[9.5px] font-semibold text-slate-900">{template.linked_workflows}</dd>
        </div>
      </dl>

      <div className="mt-2.5 flex items-center gap-1 border-t border-slate-100 pt-2.5">
        <span className="truncate text-[11px] lg:text-[8.5px] text-slate-400">
