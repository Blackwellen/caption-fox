...
/**
 * The URL-driven filter bar shared by all six Community surfaces. Every
 * control writes to the query string rather than local state, so a refresh,
 * back/forward, a copied link and a bookmarked view all restore exactly the
 * same screen. Mirrors src/components/creators/FilterBar.tsx.
 */
export default function FilterBar({
  ...
  // Reconciles local input state with the URL when it changes from outside
  // this component (back/forward, a cleared filter, a pasted link) — adjusted
  // during render per React's guidance, rather than via an effect, so typing
  // never bounces and a browser navigation is reflected immediately.
