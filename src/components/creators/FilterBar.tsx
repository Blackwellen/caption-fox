...
export interface FilterSpec {
  ...
  key: string
  label: string
  /** Placeholder shown when nothing is selected, e.g. "All Campaigns". */
  allLabel: string
  options: FilterOption[]
  /** Hidden behind "More filters" until the user opens the extra row. */
  advanced?: boolean
}
...
interface FilterBarProps {
  ...
  views?: ViewId[]
  activeView?: string
  /** Current parsed values, keyed the same way as the query string. */
  values: object
  /** Renders the date-range control between search and the selects. */
  showDateRange?: boolean
  dateRangeLabel?: string
  /** Extra controls rendered at the right end (saved views, density, etc.). */
  trailing?: React.ReactNode
  className?: string
}

/**
 * The URL-driven filter bar shared by all six Creators & UGC surfaces.
 *
 * Every control writes to the query string rather than local state, so a
 * refresh, a browser back/forward, a copied link and a bookmarked view all
 * restore exactly the same screen. Nothing here can widen the workspace scope:
 * the server re-parses and re-validates every value.
 */
export default function FilterBar({
  ...
  const [term, setTerm] = useState(String(values.q ?? ''))
