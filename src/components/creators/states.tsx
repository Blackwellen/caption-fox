...
/**
 * Canonical blocked / upgrade state for a Creators & UGC module the workspace
 * or role cannot open. Rendered by the route itself, so a pasted deep link is
...
export function CreatorsEmpty({
  title, message, action, icon = 'creators', className,
}: {
  title: string
  message: string
  action?: React.ReactNode
  icon?: 'creators' | 'search'
  className?: string
}) {
    ...
    <div className={cn(CARD, 'border-red-200 bg-red-50/50 px-4 py-3', className)} role="alert">
      <p className="flex items-start gap-2 text-[13px] text-red-800">
        ...
        <span>
          ...
          <code className="rounded bg-red-100 px-1 font-mono text-[11px]">CF-CREATORS</code>.
        </span>
      </p>
    </div>
  )
...
/** Skeleton used by every route-level `loading.tsx` under Creators & UGC. */
