...
/**
 * Shared Studio page header: breadcrumb → title → description → sub-nav.
 * Header actions are passed in per surface so each page keeps its own CTAs
 * while layout, spacing and width stay identical across all eight routes.
 */
    ...
    <header className="mb-4">
      <nav aria-label="Breadcrumb" className="mb-2">
        <ol className="flex items-center gap-1 text-xs text-slate-400">
          ...
          <li>
            <Link href="/app/studio" className="transition-colors hover:text-slate-600">Studio</Link>
          </li>
          {module !== 'overview' && (
            <>
              <li aria-hidden><ChevronRight size={12} className="text-slate-300" /></li>
