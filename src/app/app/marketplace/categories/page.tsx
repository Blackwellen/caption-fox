...
import {
  getCategories, searchProfiles, getSavedSupplierIds, getShortlistIds, getActivity,
...
import { Panel, PanelLink, StatTile, ProfileAvatar, Rating, TrustStrip } from '@/components/marketplace/module/primitives'
...
export default async function MarketplaceCategoriesPage({ searchParams }: { searchParams: Promise<RawParams> }) {
  ...
  const [categories, recommended, savedIds, shortlistIds, activity] = await Promise.all([
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
              ...
              <div className="grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                {visible.map(category => (
                  <Link
                    key={category.id}
                    href={`${MODULE_ROUTES.discover}?category=${category.slug}`}
                    className="group rounded-xl border border-slate-200 p-4 transition hover:border-blue-300 hover:shadow-[0_4px_16px_rgba(15,23,42,0.08)]"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-slate-900 group-hover:text-blue-700">{category.name}</p>
                        <p className="mt-0.5 line-clamp-2 text-[11px] leading-snug text-slate-500">{category.description}</p>
                      </div>
                    </div>
                    <dl className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2.5 text-[11px]">
                      <div>
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      ...
                      <span className="text-[11px] font-medium text-slate-500">
                        {category.avg_price_cents ? `from ${money(category.avg_price_cents)}` : '—'}
                      </span>
                    </div>
                  </Link>
                ))}
                ...
                {recommended.rows.map(profile => (
                  <li key={profile.id}>
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-xs font-semibold text-slate-800">{profile.display_name}</p>
                        <p className="truncate text-[11px] text-slate-500">{profile.headline}</p>
                      </div>
                      <div className="shrink-0 text-right">
                        <Rating value={profile.rating} count={profile.reviews_count} />
                        <p className="mt-0.5 text-[11px] font-medium text-slate-700">From {startingPrice(profile)}</p>
                      </div>
                    </Link>
                  </li>
                ))}
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {compareCategories.map(category => (
                      <tr key={category.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-3 py-2.5 text-slate-600">{category.supplier_count.toLocaleString('en-GB')}</td>
                        <td className="px-3 py-2.5 text-slate-600">{compactNumber(category.projects_count)}</td>
                        <td className="px-3 py-2.5"><Rating value={category.avg_rating} /></td>
        ...
        <div className="space-y-4">
          <div className="space-y-3">
            <StatTile icon={<Boxes size={17} />} label="Categories" value={String(categories.length)} tone="blue" hint="Verified service categories" />
            <StatTile icon={<ShieldCheck size={17} />} label="Verified suppliers" value={compactNumber(totalSuppliers)} tone="violet" hint="Across all categories" />
            <StatTile icon={<Star size={17} />} label="Average rating" value={avgRating ? avgRating.toFixed(1) : '—'} tone="emerald" hint={`${compactNumber(totalProjects)} projects completed`} />
          </div>
          ...
          <Panel title="Category performance" subtitle="Ranked by completed projects" padded={false}>
            <ol className="divide-y divide-slate-100">
              {topCategories.map((category, index) => (
                <li key={category.id}>
                  <Link href={`${MODULE_ROUTES.discover}?category=${category.slug}`} className="flex items-center gap-2.5 px-4 py-2.5 hover:bg-slate-50">
                    <span className="w-4 shrink-0 text-[11px] font-semibold text-slate-400">{index + 1}</span>
                    <span className="min-w-0 flex-1 truncate text-xs font-medium text-slate-800">{category.name}</span>
                    <span className="shrink-0 text-[11px] text-slate-500">{compactNumber(category.projects_count)} projects</span>
                  </Link>
                </li>
              ))}
            </ol>
          ...
          <Panel title="Recent activity" action={<PanelLink href={MODULE_ROUTES.overview}>View all</PanelLink>} padded={false}>
            {activity.length === 0 ? (
              <p className="p-5 text-xs text-slate-500">Category activity will appear here once you start ordering.</p>
            ) : (
              <ul className="divide-y divide-slate-100">
                {activity.map(entry => (
