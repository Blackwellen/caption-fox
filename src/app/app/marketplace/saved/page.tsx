    ...
    >
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
        <div className="min-w-0 space-y-5">
          ...
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6" aria-label="Saved summary">
            <StatTile icon={<Bookmark size={17} />} label="Saved items" value={String(items.length)} tone="blue" hint="Across all types" />
            <StatTile icon={<Users size={17} />} label="Suppliers" value={String(suppliers.length)} tone="violet" hint="Production partners" />
            <StatTile icon={<Video size={17} />} label="Creators" value={String(creators.length)} tone="emerald" hint="UGC and talent" />
            <StatTile icon={<Briefcase size={17} />} label="Services" value={String(services.length)} tone="amber" hint="Specialised services" />
            <StatTile icon={<Search size={17} />} label="Searches" value={String(searches.length)} tone="blue" hint="Saved search presets" />
            <StatTile icon={<Scale size={17} />} label="Compare eligible" value={String(compareEligible.length)} tone="slate" hint="Ready to compare" />
          </section>
          ...
          <div className="grid gap-5 lg:grid-cols-2" id="searches">
            <Panel title={`Saved searches (${searches.length})`} padded={false}>
              {searches.length === 0 ? (
                ...
                <ul className="divide-y divide-slate-100">
                  {searches.map(search => (
                    <li key={search.id} className="flex items-center gap-2 px-4 py-3">
                      ...
                      >
                        <p className="truncate text-xs font-semibold text-slate-800">{search.name}</p>
                  ...
                  {services.map(item => (
                    <li key={item.id} className="flex items-center gap-2.5 px-4 py-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-xs font-semibold text-slate-800">{item.supplier?.display_name ?? 'Service'}</p>
        ...
        <div className="space-y-4">
          ...
          <Panel title="Recent activity" action={<PanelLink href={MODULE_ROUTES.overview}>View all</PanelLink>} padded={false}>
            {activity.length === 0 ? (
              <p className="p-5 text-xs text-slate-500">Saving and comparing partners will show up here.</p>
            ) : (
              <ul className="divide-y divide-slate-100">
                {activity.map(entry => (
...
function SavedCard({
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white">
      <ProfileCover profile={profile} className="h-24" />
      <div className="flex flex-1 flex-col p-3.5">
        <Link href={`/app/marketplace/suppliers/${profile.slug}`} className="flex items-center gap-1.5 text-sm font-semibold text-slate-900 hover:text-blue-700">
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <Link
            ...
            className="inline-flex flex-1 items-center justify-center rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700"
          >
            Create request
          </Link>
        </div>
