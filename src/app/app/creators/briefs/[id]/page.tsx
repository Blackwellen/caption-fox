...
export default async function BriefDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { supabase, ctx, capabilities, access } = await requireCreatorModule('briefs')
  ...
  return (
    <div className={CREATORS_PAGE}>
      <nav aria-label="Breadcrumb" className="mb-3 flex items-center gap-1 text-[11px] text-slate-400">
        ...
        <Link href="/app/creators/briefs" className="hover:text-slate-700">Briefs</Link>
        <ChevronRight size={11} aria-hidden />
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-[12px] text-slate-500">
              {brief.campaign && <Link href={`/app/campaigns/${brief.campaign.id}`} className="hover:text-blue-600">{brief.campaign.name}</Link>}
      ...
      <div className="grid gap-4 xl:grid-cols-[1.6fr_1fr]">
        <div className="space-y-4">
          ...
          <Panel title="Creators" viewAllHref={`/app/creators/creators?status=active`}>
              ...
              <ul className="divide-y divide-slate-50">
                {creators.map(entry => (
                  <li key={entry.id} className="flex items-center justify-between py-2.5">
                    <CreatorChip creator={entry.creator} href={entry.creator ? `/app/creators/creators/${entry.creator.id}` : undefined} size={26} />
          ...
          <Panel title="Submissions" viewAllHref={`/app/creators/submissions?brief=${id}`}>
            {submissions.rows.length === 0 ? <PanelEmpty message="No submissions delivered against this brief yet." /> : (
              <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
                {submissions.rows.map(submission => (
                  <Link key={submission.id} href={`/app/creators/submissions/${submission.id}`}>
