...
export default async function PaymentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { supabase, ctx, capabilities, access } = await requireCreatorModule('payments')
  ...
  return (
    <div className={CREATORS_PAGE}>
      <nav aria-label="Breadcrumb" className="mb-3 flex items-center gap-1 text-[11px] text-slate-400">
        ...
        <Link href="/app/creators/payments" className="hover:text-slate-700">Payments</Link>
        <ChevronRight size={11} aria-hidden />
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              {capabilities.approvePayments
                ? <PaymentStatusMenu id={payment.id} current={payment.status as PaymentStatus} compact />
            ...
            <p className="mt-1 text-[13px] text-slate-500">
              {payment.brief ? <Link href={`/app/creators/briefs/${payment.brief.id}`} className="hover:text-blue-600">{payment.brief.title}</Link> : (payment.campaign?.name ?? 'No linked brief or campaign')}
            </p>
          </div>
          <CreatorChip creator={payment.creator} href={payment.creator ? `/app/creators/creators/${payment.creator.id}` : undefined} />
        </div>
