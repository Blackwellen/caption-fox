...
import {
  getCreator, listBriefs, listCreatorLists, listPayments, listRights,
...
export default async function CreatorDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [briefs, submissions, rights, payments, activity, lists] = await Promise.all([
  ...
  return (
    <div className={CREATORS_PAGE}>
      <nav aria-label="Breadcrumb" className="mb-3 flex items-center gap-1 text-[11px] text-slate-400">
        ...
        <Link href="/app/creators/creators" className="hover:text-slate-700">Creators</Link>
        <ChevronRight size={11} aria-hidden />
      ...
      <div className="grid gap-4 xl:grid-cols-[1.6fr_1fr]">
        <div className="space-y-4">
          ...
          <Panel title="Briefs" viewAllHref={`/app/creators/briefs?creator=${id}`}>
              ...
              <ul className="divide-y divide-slate-50">
                {briefs.rows.map(brief => (
                  <li key={brief.id} className="flex items-center justify-between py-2.5">
                    <Link href={`/app/creators/briefs/${brief.id}`} className="min-w-0 truncate text-[13px] font-medium text-slate-800 hover:text-blue-600">{brief.title}</Link>
          ...
          <Panel title="Submissions" viewAllHref={`/app/creators/submissions?creator=${id}`}>
            {submissions.rows.length === 0 ? <PanelEmpty message="No submissions from this creator yet." /> : (
              <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
                {submissions.rows.map(submission => (
                  <Link key={submission.id} href={`/app/creators/submissions/${submission.id}`} className="group">
          ...
          {modules.includes('rights') && (
            <Panel title="Usage Rights" viewAllHref={`/app/creators/rights?creator=${id}`}>
                  ...
                  {rights.rows.map(right => {
                    ...
                    return (
                      <li key={right.id} className="flex items-center justify-between py-2.5">
                        <Link href={`/app/creators/rights/${right.id}`} className="min-w-0 truncate text-[13px] font-medium text-slate-800 hover:text-blue-600">{right.asset_label}</Link>
          ...
          {modules.includes('payments') && capabilities.viewPayments && (
            <Panel title="Payments" viewAllHref={`/app/creators/payments?creator=${id}`}>
                ...
                <ul className="divide-y divide-slate-50">
                  {payments.rows.map(payment => (
                    <li key={payment.id} className="flex items-center justify-between py-2.5">
                      <Link href={`/app/creators/payments/${payment.id}`} className="text-[13px] font-medium text-slate-800 hover:text-blue-600">{formatMoney(payment.amount, payment.currency)}</Link>
