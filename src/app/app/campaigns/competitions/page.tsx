...
export default async function CompetitionsPage({
  ...
  const { supabase, ctx, capabilities, modules, access } = await requireCampaignModule('competitions')

  if (!access.allowed) {
    return (
      <div className={CAMPAIGN_PAGE}>
        <CampaignsHeader module="competitions" modules={modules} />
  ...
  return (
    <div className={CAMPAIGN_PAGE}>
      <CampaignsHeader
        module="competitions" modules={modules}
        actions={
          <>
      ...
      {list.rows.length === 0 ? (
        <CampaignsEmpty
          ...
          action={!filtered && capabilities.manageCompetitions
            ...
            : filtered ? <Link href="/app/campaigns/competitions" className="text-[13px] font-medium text-blue-600 hover:text-blue-700">Clear all filters</Link> : undefined}
        />
      ) : view === 'table' ? (
        <div className={cn(CARD, CARD_SHADOW, 'mb-3 overflow-x-auto')}>
          <CompetitionTable rows={list.rows} canJudge={capabilities.judge} />
        </div>
      ) : (
      ...
      <div className="mb-3 grid gap-3 xl:grid-cols-[minmax(0,1.3fr)_minmax(0,1.1fr)_minmax(0,1.2fr)_minmax(0,1.3fr)]">
        ...
        <Panel title="Top performing competitions" viewAllHref="/app/campaigns/competitions?sort=submissions_desc" viewAllLabel="View all competitions">
            ...
            <ol className="space-y-1.5">
              {aggregates.topPerformers.map((entry, index) => (
                <li key={entry.id} className="flex items-center gap-2">
                  ...
                  <Link
                    href={`/app/campaigns/competitions/${entry.id}`}
      ...
      >
        <CompetitionTable rows={list.rows.slice(0, 6)} canJudge={capabilities.judge} />
      </Panel>
    </div>
  )
}

function CompetitionTable({ rows, canJudge }: { rows: CompetitionRow[]; canJudge: boolean }) {
  if (rows.length === 0) {
        ...
        {rows.map(row => (
          <tr key={row.id} className="text-[13px] text-slate-600 transition-colors hover:bg-slate-50/70">
            <td className="px-3 py-2.5">
              <Link href={`/app/campaigns/competitions/${row.id}`} className="font-medium text-slate-900 hover:text-blue-600">
                {row.title}
              </Link>
            </td>
