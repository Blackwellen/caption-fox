...
export default async function AllCampaignsPage({
  ...
  const { supabase, ctx, userId, capabilities, modules, access } = await requireCampaignModule('all')

  if (!access.allowed) {
    return (
      <div className={CAMPAIGN_PAGE}>
        <CampaignsHeader module="all" modules={modules} />
  ...
  const workload = [
    { id: 'mine', label: 'My campaigns', value: mine.count ?? 0, href: `/app/campaigns/all?owner=${userId}` },
    { id: 'due', label: 'Due this week', value: dueThisWeek.count ?? 0, href: '/app/campaigns/all?sort=due_soonest' },
    { id: 'review', label: 'Awaiting review', value: aggregates.pendingApprovals, href: '/app/campaigns/all?approval=pending' },
    { id: 'risk', label: 'At risk / overdue', value: (aggregates.byHealth.at_risk ?? 0) + aggregates.overdue, href: '/app/campaigns/all?health=at_risk' },
  ]

  return (
    <div className={CAMPAIGN_PAGE}>
      <CampaignsHeader
        module="all" modules={modules}
        actions={
          <>
            ...
            <HeaderOverflow items={[
              { label: 'Refresh data' },
              { label: 'Campaigns overview', href: '/app/campaigns' },
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,3fr)_minmax(0,1fr)]">
          ...
          {page.rows.length === 0 ? (
            <CampaignsEmpty
              ...
              action={!filtered && capabilities.create ? <NewCampaignButton members={members} /> : (
                filtered ? <Link href="/app/campaigns/all" className="text-[13px] font-medium text-blue-600 hover:text-blue-700">Clear all filters</Link> : undefined
              )}
            />
          ) : query.view === 'table' ? (
            <CampaignTable
        ...
        <div className="min-w-0 space-y-3">
          <Panel title="Recent campaign activity" viewAllHref="/app/campaigns">
            <ActivityFeed items={activity} />
          </Panel>

          <Panel title="Workload overview" viewAllHref={`/app/campaigns/all?owner=${userId}`}>
      ...
      <Panel
        ...
        viewAllHref="/app/campaigns/all?view=table" viewAllLabel="View full table"
