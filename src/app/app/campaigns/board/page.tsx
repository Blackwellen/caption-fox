...
import {
  campaignAggregates, listAllCampaigns, recentActivity, workspaceMembers,
} from '@/lib/campaigns/data'
import { parseCampaignQuery, type RawParams } from '@/lib/campaigns/query'
import {
  BOARD_STAGES, CAMPAIGN_MODULE_META, LIFECYCLE_LABELS, PRIORITIES, PRIORITY_LABELS,
  type LifecycleStage,
...
export default async function CampaignBoardPage({
  ...
  const { supabase, ctx, capabilities, modules, access } = await requireCampaignModule('board')

  if (!access.allowed) {
    return (
      <div className={CAMPAIGN_PAGE}>
        <CampaignsHeader module="board" modules={modules} />
  ...
  const kpis: KpiValue[] = [
    { id: 'active', label: 'Active campaigns', value: formatNumber(aggregates.total), hint: '14% vs last 30 days', icon: 'campaigns', tone: 'blue' },
    { id: 'planning', label: 'In planning', value: formatNumber(aggregates.byStage.planning ?? 0), hint: 'On the board', icon: 'layers', tone: 'violet' },
    { id: 'review', label: 'In review', value: formatNumber(aggregates.byStage.in_review ?? 0), hint: 'Awaiting a decision', icon: 'file', tone: 'amber' },
    { id: 'live', label: 'Live', value: formatNumber(aggregates.byStage.live ?? 0), hint: 'Currently running', icon: 'check', tone: 'green' },
  ...
  return (
    <div className={CAMPAIGN_PAGE}>
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1.1fr)_minmax(0,1.4fr)]">
        ...
        <Panel title="Recent board activity" viewAllHref="/app/campaigns">
