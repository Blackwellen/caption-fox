...
export default async function CampaignTemplatesPage({
  ...
  const { supabase, ctx, capabilities, modules, access } = await requireCampaignModule('templates')

  if (!access.allowed) {
    return (
      <div className={CAMPAIGN_PAGE}>
        <CampaignsHeader module="templates" modules={modules} />
  ...
  return (
    <div className={CAMPAIGN_PAGE}>
      ...
      {list.rows.length === 0 ? (
        <CampaignsEmpty
          ...
          action={!filtered && capabilities.manageTemplates
            ...
            : filtered ? <Link href="/app/campaigns/templates" className="text-[13px] font-medium text-blue-600 hover:text-blue-700">Clear all filters</Link> : undefined}
        />
      ) : view === 'table' ? (
        ...
        <div className="mb-3 grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">
      ...
      <div className="mb-3 grid gap-3 xl:grid-cols-[minmax(0,1.3fr)_minmax(0,1.1fr)_minmax(0,1.1fr)_minmax(0,1.2fr)]">
        ...
        <Panel title="Most reused templates" viewAllHref="/app/campaigns/templates?sort=used_desc">
          {mostReused.length === 0 ? (
        ...
        <Panel title="Approval queue" viewAllHref="/app/campaigns/templates?status=in_review">
        ...
        <Panel title="Recent template activity" viewAllHref="/app/campaigns">
