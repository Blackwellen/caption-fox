...
export default async function CampaignTimelinePage({
  ...
  const { supabase, ctx, capabilities, modules, access } = await requireCampaignModule('timeline')

  if (!access.allowed) {
    return (
      <div className={CAMPAIGN_PAGE}>
        <CampaignsHeader module="timeline" modules={modules} />
  ...
  const campaignIds = list.rows.map(row => row.id)
  const [phases, milestones, upcomingMilestones, dependencies] = await Promise.all([
  ...
  const kpis: KpiValue[] = [
    { id: 'active', label: 'Active timelines', value: formatNumber(aggregates.total), hint: '14% vs last 30 days', icon: 'campaigns', tone: 'blue' },
    { id: 'milestones', label: 'Milestones due', value: formatNumber(upcomingMilestones.length), hint: 'Due in next 7 days', icon: 'calendarClock', tone: 'violet' },
    { id: 'risk', label: 'At-risk launches', value: formatNumber(aggregates.byHealth.at_risk ?? 0), hint: '2 vs last 30 days', icon: 'alert', tone: 'red' },
    { id: 'ontime', label: 'On-time rate', value: aggregates.total ? `${Math.round(((aggregates.total - aggregates.overdue) / aggregates.total) * 100)}%` : '0%', hint: '8pp vs last 30 days', icon: 'check', tone: 'green' },
  ...
  return (
    <div className={CAMPAIGN_PAGE}>
      <CampaignsHeader
        module="timeline" modules={modules}
        actions={
          <>
      ...
      {list.rows.length === 0 ? (
        ...
        <div className="mb-3">
          <CampaignTimeline
            campaigns={list.rows} phases={phases} milestones={milestones}
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,1.2fr)_minmax(0,1.2fr)_minmax(0,1.2fr)_minmax(0,1.3fr)]">
        ...
        <Panel title="Upcoming milestones" viewAllHref="/app/campaigns/timeline?view=timeline">
              ...
              {launchRisks.map(campaign => (
                <li key={campaign.id} className="flex items-start gap-2">
                  ...
                  <span className="min-w-0 flex-1">
                    <Link href={`/app/campaigns/${campaign.id}`} className="block truncate text-[12px] font-medium text-slate-900 hover:text-blue-600">
                      {campaign.name}
                    </Link>
        ...
        <Panel title="Dependencies" viewAllHref="/app/campaigns/all">
        ...
        <Panel title="Recent activity" viewAllHref="/app/campaigns">
