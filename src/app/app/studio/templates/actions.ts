...
export async function saveTemplate(input: TemplateInput): Promise<ActionResult> {
  ...
  if (input.id) {
    ...
    const { error: updateError } = await supabase
      .from('content_templates').update(payload).eq('id', input.id).eq('workspace_id', ctx.workspaceId)
  ...
  const { data, error: insertError } = await supabase
    ...
    .insert({ ...payload, status: 'draft', owner_id: userId, created_by: userId })
    .select('id').single()
  ...
  await logActivity(supabase, ctx.workspaceId, userId, {
    ...
    summary: `created the template “${name}”`, link: `/app/studio/templates?selected=${data.id}`,
  })
  revalidateStudio()
...
export async function setTemplateStatus(
  ...
  await logActivity(supabase, ctx.workspaceId, userId, {
    ...
    link: `/app/studio/templates?selected=${input.id}`,
  })
  revalidateStudio()
...
export async function toggleTemplateFavourite(
  ...
  if (existing) {
    ...
    revalidatePath('/app/studio/templates')
...
export async function useTemplate(
  ...
  await logActivity(supabase, ctx.workspaceId, userId, {
    ...
    summary: `used the template “${template.name}”`, link: `/app/studio/compose?id=${content.id}`,
  })
  revalidateStudio()
