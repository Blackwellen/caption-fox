...
//
// The generation call itself lives in POST /api/ai/generate (existing Anthropic
// wiring, rate-limited, credit-tracked). These actions cover the surrounding
...
export async function savePrompt(input: {
  ...
  if (input.id) {
    ...
    revalidatePath('/app/studio/ai-generate')
...
export async function setAiOutputState(
  input: { id: string; bookmarked?: boolean; status?: 'draft' | 'used' | 'discarded' },
): Promise<ActionResult> {
