...
import { csvCell } from '@/lib/creators/rules'
...
export async function GET(request: NextRequest) {
  ...
  if (entity === 'creators') {
    ...
    const { rows: listed, error } = await listCreators(supabase, ctx.workspaceId, { ...query, page: 1, size: 5000 })
    if (error) return NextResponse.json({ error }, { status: 500 })
    // Selected-row export: ids are only used to narrow the already workspace-scoped result.
    const ids = new Set(String(params.ids ?? '').split(',').filter(id => /^[0-9a-f-]{36}$/i.test(id)))
    const rows = ids.size ? listed.filter(row => ids.has(row.id)) : listed
    csv = toCsv(
