...
import {
  getSubmission, submissionAssets, submissionIssues, submissionReviews, workspaceReviewers,
...
import { UploadVersionButton } from '@/components/creators/SubmissionUpload'
import { FlagIssueButton, IssueActions, ReassignReviewer } from '@/components/creators/ReviewTools'
import {
...
export default async function SubmissionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { supabase, ctx, capabilities, access, basePath } = await requireCreatorModule('submissions')
  ...
  const [assets, reviews, issues, reviewers] = await Promise.all([
    ...
    capabilities.approve ? workspaceReviewers(supabase, ctx.workspaceId) : Promise.resolve([]),
  ])
  ...
  return (
    <div className={CREATORS_PAGE}>
      <nav aria-label="Breadcrumb" className="mb-3 flex items-center gap-1 text-[11px] text-slate-400">
        ...
        <Link href={`${basePath}/submissions`} className="hover:text-slate-700">Submissions</Link>
        <ChevronRight size={11} aria-hidden />
        ...
        <div className="space-y-4">
          <div className={`${CARD} ${CARD_SHADOW} overflow-hidden`}>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 p-4">
              ...
              <CreatorChip creator={submission.creator} href={submission.creator ? `${basePath}/creators/${submission.creator.id}` : undefined} />
            </div>
          </div>
          ...
          <Panel title="Requirement">
            <p className="text-[13px] text-slate-600">{submission.brief ? <Link href={`${basePath}/briefs/${submission.brief.id}`} className="font-medium text-blue-600 hover:underline">{submission.brief.title}</Link> : 'No linked brief.'}</p>
                ...
                {issues.map(issue => (
                  <li key={issue.id} className="flex items-center justify-between py-2.5 text-[13px]">
                    ...
                    <span className="flex items-center gap-2">
                      {capabilities.review && <IssueActions issueId={issue.id} submissionId={submission.id} status={issue.status} source={issue.source} />}
          ...
          <Panel title="Review">
            ...
            <div className="mt-4 space-y-2 border-t border-slate-100 pt-4">
              {capabilities.approve && (
                <ReassignReviewer submissionId={submission.id} current={submission.reviewer_id}
                  reviewers={reviewers.map(r => ({ id: r.id, name: r.full_name ?? r.email ?? 'Reviewer' }))} />
              )}
              {capabilities.review && <FlagIssueButton submissionId={submission.id} />}
              {capabilities.upload && (
                <UploadVersionButton submissionId={submission.id}
                  disabledReason={canTransitionSubmission(submission.status, 'waiting_review') ? null : 'Approved, rejected or published submissions cannot take a new version.'} />
              )}
            </div>
          </Panel>
