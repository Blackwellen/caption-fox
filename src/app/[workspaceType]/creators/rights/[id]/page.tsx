...
export default async function RightsDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { supabase, ctx, capabilities, access, basePath } = await requireCreatorModule('rights')
  ...
  return (
    <div className={CREATORS_PAGE}>
      <nav aria-label="Breadcrumb" className="mb-3 flex items-center gap-1 text-[11px] text-slate-400">
        ...
        <Link href={`${basePath}/rights`} className="hover:text-slate-700">Rights</Link>
        <ChevronRight size={11} aria-hidden />
      ...
      <div className={`${CARD} ${CARD_SHADOW} mb-4 p-5`}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <CreatorChip creator={rights.creator} href={rights.creator ? `${basePath}/creators/${rights.creator.id}` : undefined} />
        </div>
        ...
        <div className="space-y-4">
          ...
          {rights.submission && (
            <Panel title="Linked Submission">
              <Link href={`${basePath}/submissions/${rights.submission.id}`} className="text-[13px] font-medium text-blue-600 hover:underline">
