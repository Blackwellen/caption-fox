...
export default async function GiveawaysPage({
  ...
  const { supabase, base, ctx, capabilities, modules, access } = await requireCampaignModule('giveaways')

  if (!access.allowed) {
    return (
      <div className={CAMPAIGN_PAGE}>
        <CampaignsHeader module="giveaways" modules={modules} base={base} />
  ...
  return (
    <div className={CAMPAIGN_PAGE}>
      <CampaignsHeader
        module="giveaways" modules={modules} base={base}
        actions={
          <>
            ...
            <HeaderOverflow items={[
              ...
              { label: 'Competitions', href: `${base}/competitions` },
            ]} />
          </>
        }
      />
      ...
      {list.rows.length === 0 ? (
        <CampaignsEmpty
          ...
          action={!filtered && capabilities.manageGiveaways
            ...
            : filtered ? <Link href={`${base}/giveaways`} className="text-[13px] font-medium text-blue-600 hover:text-blue-700">Clear all filters</Link> : undefined}
        />
      ) : view === 'table' ? (
        <div className={cn(CARD, CARD_SHADOW, 'mb-3 overflow-x-auto')}>
          <GiveawayTable rows={list.rows} base={base} />
        </div>
      ) : (
      ...
      <div className="mb-3 grid gap-3 xl:grid-cols-[minmax(0,1.3fr)_minmax(0,1.1fr)_minmax(0,1.2fr)_minmax(0,1.3fr)]">
        ...
        <Panel title="Winner review queue" viewAllHref={`${base}/giveaways?extra=pending`}>
          <WinnerReviewQueue
        ...
        <Panel title="Recent giveaway activity" viewAllHref={`${base}`}>
      ...
      >
        <GiveawayTable rows={list.rows.slice(0, 6)} base={base} />
      </Panel>
    </div>
  )
}

function GiveawayTable({ rows, base }: { base: string; rows: Awaited<ReturnType<typeof listGiveaways>>['rows'] }) {
  if (rows.length === 0) {
        ...
        {rows.map(row => (
          <tr key={row.id} className="text-[13px] text-slate-600 transition-colors hover:bg-slate-50/70">
            <td className="px-3 py-2.5">
              <Link href={`${base}/giveaways/${row.id}`} className="font-medium text-slate-900 hover:text-blue-600">
                {row.title}
              </Link>
            </td>
