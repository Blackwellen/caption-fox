...
import { useCampaignsBase } from '@/components/campaigns/links'
  ...
  if (!comp) return (
    <div className="p-6">
      <EmptyState icon={Trophy} title="Competition not found"
        ...
        action={{ label: 'Back to Competitions', onClick: () => router.push(`${base}/competitions`) }} />
    </div>
  )
    ...
    <div className="p-6 max-w-screen-xl mx-auto">
      ...
      <div className="mb-6">
        <nav className="flex items-center gap-1.5 text-xs text-slate-400 mb-3">
          <a href={`${base}`} className="hover:text-slate-600">Campaigns</a>
          <span>/</span>
          <a href={`${base}/competitions`} className="hover:text-slate-600">Competitions</a>
          <span>/</span>
          <span className="text-slate-600 font-medium">{comp.title}</span>
        </nav>
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <button onClick={() => router.push(`${base}/competitions`)}
