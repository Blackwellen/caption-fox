    ...
    return (
      <div className="p-6">
        <EmptyState icon={BarChart2} title="Campaign not found" description={error}
          action={{ label: 'Back to Campaigns', onClick: () => window.location.href = `${base}` }} />
      </div>
    )
  }
    ...
    <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: campaign?.name ?? '…', href: `${base}/${id}` },
          { label: 'Analytics' },
        ]}
      >
        <Button variant="secondary" size="sm" icon={<ArrowLeft size={13} />} onClick={() => window.location.href = `${base}/${id}`}>
          Back to Campaign
        </Button>
        {campaign && (
